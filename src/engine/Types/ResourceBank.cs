...
namespace Engine.Types
{
    ...
    {
        public List<Quantified<Resource>> Contents { get; private set; }
        ...
        public ResourceBank(uint max, IEnumerable<Resource> res) : this(max)
            { (res as List<Resource>).ForEach((cur) => Add(cur)); }
        public ResourceBank(uint max, IEnumerable<(Resource, uint)> res) : this(max)
            { (res as List<(Resource, uint)>).ForEach((cur) => Add(cur.Item1, amt: cur.Item2)); }
        ...
        public void AddRange(IEnumerable<Resource> res, Action onFailure = null)
            { (res as List<Resource>).ForEach(resource => Add(resource, onFailure: onFailure)); }
        ...
        {
            if (onFailure == null)
                onFailure = () => { throw new NotEnoughOfCargoKindException(); };
        ...
        /// <returns></returns>
        public bool HasSpaceFor(uint vol)
            { return Quantity + vol <= Maximum; }
        ...
        /// <returns></returns>
        public bool HasSpaceFor(Resource res, uint amt = 1)
            { return HasSpaceFor(res.Volume * amt); }

        public bool Contains(Resource res)
        {
            foreach(Quantified<Resource> cur in Contents)
                { if (cur.Contents.Identifier == res.Identifier) return true; }
            return false;
        }
        ...
        public Quantified<Resource> this[int key] 
        { 
